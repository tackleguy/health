...
## Blocked-source recovery — 2026-09-22

- Unreadable pages now trigger public discovery using the entered name or URL slug. Exact-listing excerpts are preferred; up to two matching alternate pages can be read. Recovered values require review before application, with source links and persistent provenance notes. Category pages and different model/size identifiers cannot become automatic recovery matches.
- Added public DuckDuckGo discovery alongside Bing and existing manufacturer catalogs. Provider errors degrade to editable fields and a browser-local pasted-specification extractor. No challenge solving, proxy bypass, model download, or generated numerical estimates are involved.
- Added exact-item parsing for Walmart's initial page data, including weight, explicit price/currency, model, capacity, materials and labelled packed size. Recommendations and generated summaries are excluded. Live extraction of item `5128730697` produced its nine available specification fields; a real public search excerpt also yielded the same listing's carry weight. Prices remain retrieval-time snapshots.
- Browser checks exercised direct Walmart autofill, recovery from an intentionally unavailable example.com URL to a specific matching listing, rejection of an overly broad category match, pasted-specification extraction, explicit review/application, retained manual fields, visible unverified notes, and focus return. No inventory was saved or modified.
- Desktop (1280px) and mobile (390px) had no horizontal overflow; the pasted-specification textarea used 16px text. The expanded mobile extraction state passed axe with 0 violations, 0 manual-review entries, and 44 passes. Temporary viewport settings were reset.
- All 60 regression tests passed, including nine recovery tests. Scoped ESLint, TypeScript, whitespace checks and the production webpack build passed. Account database persistence remains unverified locally. These recovery changes were verified locally before publishing.
