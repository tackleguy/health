import { cleanShopping } from "./shopping";
...
export function cleanRequest(value: unknown): TripRequest {
  ...
  const place = object(r.place);
  const cleanPlace = str(place.id) && str(place.name) && num(place.latitude, -90, 90) !== null && num(place.longitude, -180, 180) !== null ? { id: str(place.id, 180), name: str(place.name, 250), latitude: Number(place.latitude), longitude: Number(place.longitude) } : null;
  return { ...EMPTY_REQUEST, locationMode: r.locationMode === "near" ? "near" : "in", radiusKm: [10, 25, 50, 100, 200].includes(Number(r.radiusKm)) ? Number(r.radiusKm) : 50, place: r.locationMode === "near" ? cleanPlace : null, distanceMiles: num(r.distanceMiles, 0.1), days: num(r.days, 1, 365) === null ? null : Math.floor(Number(r.days)), region: str(r.region, 100), startDate: /^\d{4}-\d{2}-\d{2}$/.test(str(r.startDate)) ? str(r.startDate) : "", lowTempF: num(r.lowTempF, -80, 130), waterLiters: num(r.waterLiters, 0, 100), foodOzPerDay: num(r.foodOzPerDay, 0, 300), fuelOz: num(r.fuelOz, 0, 1000), suppliesInGear: r.suppliesInGear === true };
}
export function parseMemory(raw: string | null): PlannerMemory {
  ...
  try {
    ...
    const plans: SavedPlan[] = (Array.isArray(m.plans) ? m.plans : []).slice(0, 30).flatMap(value => {
      ...
      return [{ id: str(plan.id), prompt: str(plan.prompt, 1500), request: cleanRequest(plan.request), route: route.id && route.name && num(route.distanceMiles, 0.1) ? { id: str(route.id), name: str(route.name, 180), region: str(route.region, 200), distanceMiles: Number(route.distanceMiles), elevationFt: num(route.elevationFt, 0, 100000), difficulty: str(route.difficulty, 30), sourceUrl: /^https:\/\//i.test(str(route.sourceUrl)) ? str(route.sourceUrl, 2000) : null, sourceLabel: str(route.sourceLabel, 100), ...(route.kind === "segment" ? { kind: "segment" as const } : {}), ...(/^\/explore\/trails\/[a-zA-Z0-9:{}_-]+$/.test(str(route.catalogHref)) ? { catalogHref: str(route.catalogHref) } : {}), note: str(route.note), ...(typeof route.trailId === "string" ? { trailId: str(route.trailId, 160) } : {}) } : null, gear: (Array.isArray(plan.gear) ? plan.gear : []).map(cleanGear).filter((g): g is PlannerGear => Boolean(g)).slice(0, 500), packedIds: (Array.isArray(plan.packedIds) ? plan.packedIds : []).filter((id): id is string => typeof id === "string").slice(0, 500), shopping: cleanShopping(plan.shopping), savedAt: str(plan.savedAt), feedback: ["easy", "right", "hard"].includes(String(feedback.effort)) && ["comfortable", "too-heavy"].includes(String(feedback.packComfort)) ? { effort: feedback.effort as "easy"|"right"|"hard", packComfort: feedback.packComfort as "comfortable"|"too-heavy", carriedLb: num(feedback.carriedLb, 1, 150), notes: str(feedback.notes, 500) } : null }];
    });
