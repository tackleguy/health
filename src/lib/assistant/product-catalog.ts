...
export function lookupProductCatalog(raw: string): ProductResearch | undefined {
  ...
  return { title: "Ozark Trail 1-Person Backpacking Tent", url: OZARK_SOURCE, retrievedAt: CHECKED_AT, facts, variants: [], excerpt: facts.map(f => `${f.label}: ${f.value}`).join("\n"), recovery: { method: "catalog", requestedUrl: raw, notice: "Found this exact item in HikeSync’s verified public catalog. Specifications were checked September 24, 2026 UTC. The price is a dated observation and may have changed. Review the details, then fill out parameters.", sources: [{ title: "Walmart · Ozark Trail W2201", url: OZARK_SOURCE, snippet: "Published specifications for Walmart item 5128730697, checked September 24, 2026." }] } };
}
