...
test("a bounded map represents every matching record, not one results page", async () => {
  const data = await searchCatalogMap();
  assert.equal(data.total, 90000);
  assert.ok(data.points.length <= MAX_MAP_POINTS);
  assert.equal(data.points.reduce((sum, point) => sum + point.count, 0), 90000);
