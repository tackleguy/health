...
export function LocalGear() {
  const { memory, update, error } = usePlannerMemory(null);
  ...
  return <div className="planner-shell">
    ...
    </section><aside className="planner-sidebar local-gear-aside"><h2>Know your equipment.</h2><p>{memory.gear.length} items · {(knownOz / 16).toFixed(1)} lb of known inventory weight{unknown ? ` · ${unknown} missing ${unknown === 1 ? "weight" : "weights"}` : ""}</p><p>Your trip’s pack weight uses only the items you select, plus food, water and fuel. Worn items are counted separately.</p><hr /><h3>Weights with a source</h3><p>Look up a product name or paste its manufacturer’s page in the equipment editor. Review specifications before adding them.</p><hr /><h3>Your browser, your gear</h3><p>This guest list is shared with your local planner. Sign in to work with account gear; guest and account inventories stay separate.</p><Link className="planner-button secondary" href="/login?next=/gear">Sign in to account gear</Link></aside></div>
  </div>;
}
